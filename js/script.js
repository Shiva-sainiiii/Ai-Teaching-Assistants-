...
// 1. Firebase Imports (getDoc add kiya gaya hai user verification ke liye)
import { initializeApp } from "https://www.gstatic.com/firebasejs/12.11.0/firebase-app.js";
import { getFirestore, doc, setDoc, getDoc, updateDoc, arrayUnion, increment } from "https://www.gstatic.com/firebasejs/12.11.0/firebase-firestore.js";
import { getAnalytics } from "https://www.gstatic.com/firebasejs/12.11.0/firebase-analytics.js";
...
// 3. 🔑 Auth & Device ID Setup (Ab ye username based chalega)
let currentUsername = localStorage.getItem("duke_username") || "";

function getUserDocRef() {
    return doc(db, "user_data", currentUsername || "anonymous_user");
}

// 🔥 Firebase Sync Functions (Dynamic document reference use kar rahe hain)
async function updateFirebasePDFCount() {
    if (!currentUsername) return;
    await setDoc(getUserDocRef(), { pdfCount: increment(1) }, { merge: true });
}

async function saveTopicToFirebase(topic) {
    if (!currentUsername) return;
    await setDoc(getUserDocRef(), { topics: arrayUnion(topic) }, { merge: true });
}

async function saveQuizToFirebase(score, total) {
    if (!currentUsername) return;
    const percentage = Math.round((score / total) * 100);
    await setDoc(getUserDocRef(), { quizData: arrayUnion(percentage) }, { merge: true });
}

async function saveStudyTimeToFirebase(minutes) {
    if (!currentUsername) return;
    await setDoc(getUserDocRef(), { studyTime: increment(minutes) }, { merge: true });
}
...
// --- 🔐 CUSTOM AUTH SYSTEM (LOGIN / SIGNUP) ---
...
function loginSuccess(username) {
    ...
    localStorage.setItem("duke_username", username);
    if (authPage) authPage.classList.remove("active");
    
    // Default document page kholna
    const pdfPage = document.getElementById("pdfPage");
    const pdfNavBtn = document.querySelector('[data-page="pdfPage"]');
    if (pdfPage) pdfPage.classList.add("active");
    if (pdfNavBtn) pdfNavBtn.classList.add("active");
...
// Auto Login check
if (currentUsername) {
    setTimeout(() => loginSuccess(currentUsername), 100);
}
...
if (signupBtn) {
    signupBtn.addEventListener("click", async () => {
        ...
        if (!user || !pass) { 
            authError.innerText = "Fill all fields!"; 
            authError.style.display = "block"; 
            return; 
        }

        const userRef = doc(db, "auth_users", user);
        ...
        if (userSnap.exists()) {
            ...
            await setDoc(userRef, { password: pass, createdAt: new Date() });
            loginSuccess(user);
        }
    });
}
...
async function handleFileUpload(e) {
    ...
    const statusDiv = document.getElementById("uploadStatus");
    const uploadText = document.getElementById("uploadText");
    const textPreviewPanel = document.getElementById("textPreview");
    const textContentArea = document.getElementById("textContent");

    uploadText.innerText = "Processing file...";
    statusDiv.style.display = "none";
    if (textPreviewPanel) textPreviewPanel.style.display = "none";
    ...
    try {
        if (file.type === "application/pdf") {
            ...
            reader.onload = async function () {
                ...
                // Save text to Firestore & Show preview
                await finalizeExtractedText(file.name, pdfText);
                increasePDFCount();
            };
            reader.readAsArrayBuffer(file);
...
// 💾 Save extracted text in Firestore and display on screen
async function finalizeExtractedText(fileName, text) {
    const textPreviewPanel = document.getElementById("textPreview");
    const textContentArea = document.getElementById("textContent");

    // 1. Show UI preview
    if (textPreviewPanel && textContentArea) {
        textContentArea.innerText = text;
        textPreviewPanel.style.display = "block";
    }

    // 2. Save in Firestore permanently
    if (currentUsername) {
        const docRef = doc(db, "users", currentUsername, "my_library", fileName);
        await setDoc(docRef, {
            text: text,
            fileName: fileName,
            createdAt: new Date()
        });
        console.log("Text saved in library.");
    }
}
...
function renderInteractiveQuiz(data) {
    ...
    headerCard.className = "quiz-header-card";
...
function saveQuiz(score, totalQuestions) {
    let quizData = JSON.parse(localStorage.getItem("quizData")) || [];
    let percentage = Math.round((score / totalQuestions) * 100);
    quizData.push(percentage);
    localStorage.setItem("quizData", JSON.stringify(quizData));
    saveQuizToFirebase(percentage);
}

// --- 3. RELEVANT TEXT EXTRACTOR (RAG Logic) ---
function getRelevantText(pdfText, text) {
    if (!pdfText) return "";
...
// --- 4. OCR FALLBACK FUNCTIONS (Using Tesseract.js) ---
async function extractTextFromImage(file) {
    try {
...
// --- 6. VOICE FEATURE LOGIC (Optimized for Hindi) ---
const micBtn = document.getElementById("micBtn");
const inputField = document.getElementById("input");
...
// --- 8. FIREBASE BACKGROUND SYNC HELPERS ---
async function saveTopicToFirebase(topic) {
    try {
    ...
    } catch (e) { console.error("Firebase topic save error:", e); }
}

async function saveQuizToFirebase(percentage) {
    try {
        const userRef = doc(db, "users", userId);
        await updateDoc(userRef, {
            quizScores: arrayUnion(percentage)
        });
    } catch (e) { console.error("Firebase quiz save error:", e); }
}
...
async function saveStudyTimeToFirebase(minutes) {
    try {
        const userRef = doc(db, "users", userId);
        await updateDoc(userRef, {
            studyTime: increment(minutes)
        });
    } catch (e) { console.error("Firebase study time save error:", e); }
}
